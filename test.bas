Public Sub test()
    ...
    ' Krok 4: wstawiamy nagłówek "Ilosc" w pierwszej wolnej kolumnie.
    Dim colIlosc As Long
    Dim colIloscL As String
    colIlosc = lastCol + 1
    ws.Cells(headerRow, colIlosc).Value = "Ilosc"
    ...
    ' Krok 8: wyliczamy "Ilosc" z NazwaLoga (po znaku ")").
    Dim i As Long
    ...
    For i = 1 To UBound(arrSkrot, 1)
        ...
        If UCase$(Trim$(sk)) = "KOMPUTER" Then
            If InStr(1, nameVal, "ZWROT TOWARU", vbTextCompare) > 0 Then
                Dim zwrotCount As String
                Dim zwrotPos As Long
                Dim beforeZwrot As String
                Dim zwrotIdx As Long
                Dim zwrotCh As String

                zwrotPos = InStr(1, nameVal, "ZWROT TOWARU", vbTextCompare)
                beforeZwrot = Trim$(Left$(nameVal, zwrotPos - 1))
                zwrotCount = ""

                For zwrotIdx = Len(beforeZwrot) To 1 Step -1
                    zwrotCh = Mid$(beforeZwrot, zwrotIdx, 1)
                    If zwrotCh >= "0" And zwrotCh <= "9" Then
                        zwrotCount = zwrotCh & zwrotCount
                    ElseIf Len(zwrotCount) > 0 Then
                        Exit For
                    ElseIf zwrotCh <> " " Then
                        Exit For
                    End If
                ...
                If Len(zwrotCount) > 0 Then
                    arrIlosc(i, 1) = CDbl(zwrotCount)
                End If
            ElseIf InStr(1, nameVal, "PRZYJĘCIE TOWARU", vbTextCompare) > 0 Then
                Dim przyjecieCount As String
                Dim przyjeciePos As Long
                Dim beforePrzyjecie As String
                Dim przyjecieIdx As Long
                Dim przyjecieCh As String

                przyjeciePos = InStr(1, nameVal, "PRZYJĘCIE TOWARU", vbTextCompare)
                beforePrzyjecie = Trim$(Left$(nameVal, przyjeciePos - 1))
                przyjecieCount = ""

                For przyjecieIdx = Len(beforePrzyjecie) To 1 Step -1
                    przyjecieCh = Mid$(beforePrzyjecie, przyjecieIdx, 1)
                    If przyjecieCh >= "0" And przyjecieCh <= "9" Then
                        przyjecieCount = przyjecieCh & przyjecieCount
                    ElseIf Len(przyjecieCount) > 0 Then
                        Exit For
                    ElseIf przyjecieCh <> " " Then
                        Exit For
                    End If
                Next przyjecieIdx

                If Len(przyjecieCount) > 0 Then
                    arrIlosc(i, 1) = CDbl(przyjecieCount)
                End If
            End If
            If InStr(1, nameVal, "ZWROT TOWARU", vbTextCompare) > 0 Then
                arrPozycje(i, 1) = "Komputer - zwrot towaru"
            ElseIf InStr(1, nameVal, "PRZYJĘCIE TOWARU", vbTextCompare) > 0 Then
                arrPozycje(i, 1) = "Komputer - przyjęcie towaru PZ"
            ElseIf InStr(1, nameVal, "STATUS PACZKI", vbTextCompare) > 0 Then
                arrPozycje(i, 1) = "Komputer - zwrot paczki"
            ElseIf InStr(1, nameVal, "SPRAWDZENIE LOKALIZACJI TOWARU", vbTextCompare) > 0 Then
                arrPozycje(i, 1) = "Komputer - Sprawdzenie lokalizacji"
            Else
                arrPozycje(i, 1) = ""
            End If
            prevSkLinked = False
