...
'=========================================================
' BLOK 5 (KORB): korekta procesu dla długich serii "Skanowanie skrzynki"
'   - kolumna G = "Skanowanie skrzynki" (ciągłe wiersze)
'   - ten sam operator w kolumnie D
'   - jeśli seria > 50 wierszy → w kolumnie H wpisz "Sortowanie skrzynek"
'=========================================================
Dim arrDKORB2 As Variant, arrGKORB2 As Variant, arrHKORB2 As Variant
Dim rKORB5 As Long
Dim startKORB5 As Long
Dim opKORB5 As String
Dim countKORB5 As Long

arrDKORB2 = wsKORB.Range("D1:D" & lastRow2KORB).Value
arrGKORB2 = wsKORB.Range("G1:G" & lastRow2KORB).Value
arrHKORB2 = wsKORB.Range("H1:H" & lastRow2KORB).Value

rKORB5 = 1
Do While rKORB5 <= lastRow2KORB
    If Trim$(CStr(arrGKORB2(rKORB5, 1))) = "Skanowanie skrzynki" Then
        startKORB5 = rKORB5
        opKORB5 = Trim$(CStr(arrDKORB2(rKORB5, 1)))
        rKORB5 = rKORB5 + 1

        Do While rKORB5 <= lastRow2KORB _
            And Trim$(CStr(arrGKORB2(rKORB5, 1))) = "Skanowanie skrzynki" _
            And Trim$(CStr(arrDKORB2(rKORB5, 1))) = opKORB5
            rKORB5 = rKORB5 + 1
        Loop

        countKORB5 = rKORB5 - startKORB5
        If countKORB5 > 50 Then
            Dim iKORB5 As Long
            If startKORB5 > 1 Then
                If Trim$(CStr(arrDKORB2(startKORB5 - 1, 1))) = opKORB5 And _
                   Trim$(CStr(arrGKORB2(startKORB5 - 1, 1))) = "Rozdzielanie skrzynek" Then
                    arrHKORB2(startKORB5 - 1, 1) = "Sortowanie skrzynek"
                End If
            End If
            For iKORB5 = startKORB5 To rKORB5 - 1
                arrHKORB2(iKORB5, 1) = "Sortowanie skrzynek"
            Next iKORB5
            If rKORB5 <= lastRow2KORB Then
                If Trim$(CStr(arrDKORB2(rKORB5, 1))) = opKORB5 And _
                   Trim$(CStr(arrGKORB2(rKORB5, 1))) = "Rozdzielanie skrzynek" Then
                    arrHKORB2(rKORB5, 1) = "Sortowanie skrzynek"
                End If
            End If
        End If
    Else
...
wsKORB.Range("H1:H" & lastRow2KORB).Value = arrHKORB2
...
'WstawNaglowkiZakresu
    ...
    '<<<<<<<<<<<<<<<<<<<<<<<<< Zakres sesji dobowej i zmianowej >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
    Dim headerRowR As Long
    Dim colR As Long
    Dim lastRowR As Long
    Dim rowIndexR As Long
    Dim lastColR As Long
    Dim operatorValueR As String
    Dim seenOperatorsR As Object
    Dim thresholdR As Double
    Dim dtStartR As Double
    Dim dtEndR As Double
    Dim dateValueR As Variant
    Dim timeValueR As Variant
    Dim dataRangeR As Range
    Dim dataArrR As Variant
    Dim outputArrR() As Variant
    Dim blockStartR As Long
    Dim blockHasSMinusR As Boolean
    Dim previousOperatorR As String
    Dim nightInfoR As Object
    Dim nightKeyR As Variant
    Dim infoArrNightR As Variant
    Dim dateOnlyR As Double
    Dim timeOnlyR As Double
    Dim startDateR As Double
    Dim startRowR As Long
    Dim lastRowWindowR As Long
    Dim earliestTimeR As Double
    Dim earliestRowR As Long

    headerRowR = 1
    colR = 1
    thresholdR = 7# / 24#

    lastRowR = wsR.Cells(wsR.Rows.Count, "D").End(xlUp).Row
    lastColR = wsR.Cells(headerRowR, wsR.Columns.Count).End(xlToLeft).Column
    If wsR.Cells(headerRowR, lastColR).Value = "" Then
        lastColR = 0
    End If
    colR = lastColR + 1

    Set dataRangeR = wsR.Range(wsR.Cells(headerRowR, 1), wsR.Cells(lastRowR, 11))
    dataArrR = dataRangeR.Value

    ReDim outputArrR(1 To lastRowR, 1 To 1)
    outputArrR(1, 1) = "SesjaZmianowaTok"

    Set seenOperatorsR = CreateObject("Scripting.Dictionary")

    blockStartR = headerRowR + 1
    blockHasSMinusR = False
    previousOperatorR = Trim(CStr(dataArrR(blockStartR, 4)))

    For rowIndexR = headerRowR + 1 To lastRowR
        operatorValueR = Trim(CStr(dataArrR(rowIndexR, 4)))
        If CStr(dataArrR(rowIndexR, 11)) = "S-" Then
            blockHasSMinusR = True
        End If
        If operatorValueR <> "" Then
            If Not seenOperatorsR.Exists(operatorValueR) Then
                outputArrR(rowIndexR, 1) = "Start"
                seenOperatorsR.Add operatorValueR, True
            End If
        End If

        If operatorValueR <> previousOperatorR And previousOperatorR <> "" Then
            If Not blockHasSMinusR Then
                outputArrR(rowIndexR - 1, 1) = "Koniec"
            End If
            blockStartR = rowIndexR
            blockHasSMinusR = (CStr(dataArrR(rowIndexR, 11)) = "S-")
            previousOperatorR = operatorValueR
        End If
    Next rowIndexR

    If previousOperatorR <> "" And Not blockHasSMinusR Then
        outputArrR(lastRowR, 1) = "Koniec"
    End If

    For rowIndexR = headerRowR + 1 To lastRowR - 3
        If CStr(dataArrR(rowIndexR, 11)) = "S-" And _
           CStr(dataArrR(rowIndexR + 3, 11)) = "S-" Then

            dateValueR = dataArrR(rowIndexR, 1)
            timeValueR = dataArrR(rowIndexR, 2)
            dtStartR = 0#
            If IsNumeric(dateValueR) Then
                dtStartR = CDbl(dateValueR)
            ElseIf IsDate(dateValueR) Then
                dtStartR = CDbl(CDate(dateValueR))
            End If
            If IsNumeric(timeValueR) Then
                dtStartR = dtStartR + CDbl(timeValueR)
            ElseIf IsDate(timeValueR) Then
                dtStartR = dtStartR + CDbl(CDate(timeValueR))
            End If

            dateValueR = dataArrR(rowIndexR + 3, 1)
            timeValueR = dataArrR(rowIndexR + 3, 2)
            dtEndR = 0#
            If IsNumeric(dateValueR) Then
                dtEndR = CDbl(dateValueR)
            ElseIf IsDate(dateValueR) Then
                dtEndR = CDbl(CDate(dateValueR))
            End If
            If IsNumeric(timeValueR) Then
                dtEndR = dtEndR + CDbl(timeValueR)
            ElseIf IsDate(timeValueR) Then
                dtEndR = dtEndR + CDbl(CDate(timeValueR))
            End If

            If dtEndR - dtStartR > thresholdR Then
                outputArrR(rowIndexR, 1) = "Koniec"
            End If
        End If
    Next rowIndexR

    Set nightInfoR = CreateObject("Scripting.Dictionary")
    ...
    For rowIndexR = headerRowR + 1 To lastRowR
        ...
        If operatorValueR <> "" Then
            ...
            timeOnlyR = 0#
            If IsNumeric(dateValueR) Then
                dateOnlyR = Int(CDbl(dateValueR))
            ElseIf IsDate(dateValueR) Then
                dateOnlyR = Int(CDbl(CDate(dateValueR)))
            End If
            If IsNumeric(timeValueR) Then
                timeOnlyR = CDbl(timeValueR)
            ElseIf IsDate(timeValueR) Then
                timeOnlyR = CDbl(CDate(timeValueR)) - Int(CDbl(CDate(timeValueR)))
            End If

            If Not nightInfoR.Exists(operatorValueR) Then
                infoArrNightR = Array(0, 0#, 0, 0, timeOnlyR, rowIndexR)
                nightInfoR.Add operatorValueR, infoArrNightR
            Else
                infoArrNightR = nightInfoR(operatorValueR)
                If timeOnlyR < infoArrNightR(4) Then
                    infoArrNightR(4) = timeOnlyR
                    infoArrNightR(5) = rowIndexR
                End If
            End If

            If infoArrNightR(0) = 0 Then
                If timeOnlyR >= (16# / 24#) Then
                    infoArrNightR(0) = 1
                    infoArrNightR(1) = dateOnlyR
                    infoArrNightR(2) = rowIndexR
                    infoArrNightR(3) = 0
                    nightInfoR(operatorValueR) = infoArrNightR
                End If
            Else
                startDateR = infoArrNightR(1)
                If dateOnlyR = startDateR + 1 And timeOnlyR <= (8# / 24#) Then
                    infoArrNightR(3) = rowIndexR
                    nightInfoR(operatorValueR) = infoArrNightR
                End If
            End If
        End If
    ...
    For Each nightKeyR In nightInfoR.Keys
        infoArrNightR = nightInfoR(nightKeyR)
        startDateR = infoArrNightR(1)
        startRowR = infoArrNightR(2)
        lastRowWindowR = infoArrNightR(3)
        earliestTimeR = infoArrNightR(4)
        earliestRowR = infoArrNightR(5)

        If lastRowWindowR <> 0 And startRowR <> 0 Then
            Dim hasKoniecR As Boolean
            hasKoniecR = False
            For rowIndexR = headerRowR + 1 To lastRowR
                If Trim(CStr(dataArrR(rowIndexR, 4))) = nightKeyR Then
                    If outputArrR(rowIndexR, 1) = "Koniec" Then
                        hasKoniecR = True
                        Exit For
                    End If
                End If
            ...
            If hasKoniecR Then
                GoTo NextNightKeyR
            End If
            If earliestTimeR >= (5# / 24#) And earliestTimeR <= (8# / 24#) Then
                outputArrR(earliestRowR, 1) = "Start"
            Else
            For rowIndexR = headerRowR + 1 To lastRowR
                If Trim(CStr(dataArrR(rowIndexR, 4))) = nightKeyR Then
                    If outputArrR(rowIndexR, 1) = "Start" And rowIndexR <> startRowR Then
                        outputArrR(rowIndexR, 1) = ""
                    End If
                End If
            Next rowIndexR
            outputArrR(startRowR, 1) = "Start"
            End If
            outputArrR(lastRowWindowR, 1) = "Koniec"
        End If
NextNightKeyR:
    Next nightKeyR

    Dim startRowsR As Object
    Dim endRowsR As Object
    Dim startLookupR As Variant
    Dim endLookupR As Variant
    Dim colDobowaR As Long
    Dim colDobowaLR As String
    Dim arrDobowaR() As Variant
    Dim minRR As Double, maxRR As Double
    Dim minPR As Double, maxPR As Double
    Dim maxNAR As Double, minNBR As Double
    Dim sesjaDobowaR As String
    Dim dictStartR As Object
    Dim dictDatesR As Object
    Dim dictInfoR As Object
    Dim infoArrDobowaR As Variant
    Dim minDateR As Double
    Dim dateKeyR As String
    Dim startKeyR As String
    Dim lastTimeR As Double
    Dim switchedNBR As Boolean
    Dim prevDateOnlyR As Double
    Dim prevDateValueR As Variant

    Set startRowsR = CreateObject("Scripting.Dictionary")
    Set endRowsR = CreateObject("Scripting.Dictionary")

    For rowIndexR = headerRowR + 1 To lastRowR
        operatorValueR = Trim(CStr(dataArrR(rowIndexR, 4)))
        If operatorValueR <> "" Then
            If outputArrR(rowIndexR, 1) = "Start" Then
                startRowsR(operatorValueR) = rowIndexR
            ElseIf outputArrR(rowIndexR, 1) = "Koniec" Then
                endRowsR(operatorValueR) = rowIndexR
            End If
        End If
    Next rowIndexR

    For Each startLookupR In startRowsR.Keys
        If endRowsR.Exists(startLookupR) Then
            startRowR = startRowsR(startLookupR)
            lastRowWindowR = endRowsR(startLookupR)
            If lastRowWindowR > startRowR Then
                For rowIndexR = startRowR + 1 To lastRowWindowR - 1
                    If Trim(CStr(dataArrR(rowIndexR, 4))) = startLookupR Then
                        If outputArrR(rowIndexR, 1) = "" Then
                            outputArrR(rowIndexR, 1) = "W toku"
                        End If
                    End If
                Next rowIndexR
            End If
    ...
    colDobowaR = colR + 1
    wsR.Cells(headerRowR, colDobowaR).Value = "SesjaDobowaZakres"
    colDobowaLR = Split(wsR.Cells(headerRowR, colDobowaR).Address(False, False), CStr(headerRowR))(0)

    minRR = TimeSerial(4, 0, 0)
    maxRR = TimeSerial(9, 0, 0)
    minPR = TimeSerial(9, 0, 0)
    maxPR = TimeSerial(16, 0, 0)
    maxNAR = TimeSerial(4, 0, 0)
    minNBR = TimeSerial(16, 0, 0)

    ReDim arrDobowaR(1 To lastRowR, 1 To 1)
    arrDobowaR(1, 1) = "SesjaDobowaZakres"

    Set dictDatesR = CreateObject("Scripting.Dictionary")
    minDateR = 0#
    ...
    For rowIndexR = headerRowR + 1 To lastRowR
        ...
        If dateOnlyR <> 0# Then
            dateKeyR = CStr(dateOnlyR)
            If Not dictDatesR.Exists(dateKeyR) Then
                dictDatesR.Add dateKeyR, True
                If minDateR = 0# Or dateOnlyR < minDateR Then
                    minDateR = dateOnlyR
                End If
            End If
        End If
    ...
    Set dictStartR = CreateObject("Scripting.Dictionary")
    Set dictInfoR = CreateObject("Scripting.Dictionary")
    ...
    For rowIndexR = headerRowR + 1 To lastRowR
        ...
        startKeyR = CStr(dateOnlyR) & "|" & operatorValueR
        If operatorValueR <> "" And dateOnlyR <> 0# And dateOnlyR = minDateR Then
            If CStr(dataArrR(rowIndexR, 11)) = "S+" Then
                Dim sameSeriesR As Boolean
                sameSeriesR = False
                If rowIndexR > headerRowR + 1 Then
                    prevDateOnlyR = 0#
                    prevDateValueR = dataArrR(rowIndexR - 1, 1)
                    If IsNumeric(prevDateValueR) Then
                        prevDateOnlyR = Int(CDbl(prevDateValueR))
                    ElseIf IsDate(prevDateValueR) Then
                        prevDateOnlyR = Int(CDbl(CDate(prevDateValueR)))
                    End If
                    If CStr(dataArrR(rowIndexR - 1, 11)) = "S+" And _
                       Trim$(CStr(dataArrR(rowIndexR - 1, 4))) = operatorValueR And _
                       prevDateOnlyR = dateOnlyR Then
                        sameSeriesR = True
                    End If
                End If

                If Not sameSeriesR Then
                    If dictInfoR.Exists(startKeyR) Then
                        infoArrDobowaR = dictInfoR(startKeyR)
                        If CStr(infoArrDobowaR(0)) = "N-B" Then
                            arrDobowaR(rowIndexR, 1) = "N-B"
                            GoTo NextDobowaRowR
                        End If
                    End If
                End If
                arrDobowaR(rowIndexR, 1) = ""
                GoTo NextDobowaRowR
            End If
            If IsNumeric(dataArrR(rowIndexR, 2)) Then
                timeOnlyR = CDbl(dataArrR(rowIndexR, 2))
                If Not dictStartR.Exists(startKeyR) Then
                    If timeOnlyR <= maxNAR Then
                        sesjaDobowaR = "N-A"
                    ElseIf timeOnlyR > minRR And timeOnlyR <= maxRR Then
                        sesjaDobowaR = "R"
                    ElseIf timeOnlyR > minPR And timeOnlyR <= maxPR Then
                        sesjaDobowaR = "P"
                    Else
                        sesjaDobowaR = "N-B"
                    End If
                    dictStartR.Add startKeyR, True
                    dictInfoR.Add startKeyR, Array(sesjaDobowaR, timeOnlyR, False)
                Else
                    infoArrDobowaR = dictInfoR(startKeyR)
                    sesjaDobowaR = CStr(infoArrDobowaR(0))
                    lastTimeR = CDbl(infoArrDobowaR(1))
                    switchedNBR = CBool(infoArrDobowaR(2))

                    If Not switchedNBR And (timeOnlyR - lastTimeR) > (8# / 24#) Then
                        sesjaDobowaR = "N-B"
                        switchedNBR = True
                    End If

                    infoArrDobowaR(0) = sesjaDobowaR
                    infoArrDobowaR(1) = timeOnlyR
                    infoArrDobowaR(2) = switchedNBR
                    dictInfoR(startKeyR) = infoArrDobowaR
                End If
                arrDobowaR(rowIndexR, 1) = sesjaDobowaR
            End If
        End If
NextDobowaRowR:
    Next rowIndexR

    wsR.Cells(headerRowR, colR).Resize(lastRowR, 1).Value = outputArrR
    wsR.Cells(headerRowR, colDobowaR).Resize(lastRowR, 1).Value = arrDobowaR

    Dim dictFirstNAR As Object
    Dim rowListR() As Long
    Dim listCountR As Long
    Dim idxR As Long
    Dim opNAR As String
    Dim tempRowR As Long
    Dim dtOrigNAR As Double
    Dim dtNewNAR As Double
    Dim vDateNAR As Variant
    Dim vTimeNAR As Variant

    Set dictFirstNAR = CreateObject("Scripting.Dictionary")
    listCountR = 0

    For rowIndexR = headerRowR + 1 To lastRowR
        If CStr(arrDobowaR(rowIndexR, 1)) = "N-A" Then
            opNAR = Trim$(CStr(dataArrR(rowIndexR, 4)))
            If opNAR <> "" Then
                If Not dictFirstNAR.Exists(opNAR) Then
                    dictFirstNAR.Add opNAR, rowIndexR
                    listCountR = listCountR + 1
                    ReDim Preserve rowListR(1 To listCountR)
                    rowListR(listCountR) = rowIndexR
                End If
            End If
        End If
    Next rowIndexR

    If listCountR > 1 Then
        For rowIndexR = 1 To listCountR - 1
            For idxR = rowIndexR + 1 To listCountR
                If rowListR(rowIndexR) < rowListR(idxR) Then
                    tempRowR = rowListR(rowIndexR)
                    rowListR(rowIndexR) = rowListR(idxR)
                    rowListR(idxR) = tempRowR
                End If
            Next idxR
        Next rowIndexR
    End If

    For idxR = 1 To listCountR
        rowIndexR = rowListR(idxR)
        wsR.Rows(rowIndexR).Insert shift:=xlDown
        wsR.Rows(rowIndexR + 1).Copy
        wsR.Rows(rowIndexR).PasteSpecial Paste:=xlPasteValuesAndNumberFormats
        wsR.Cells(rowIndexR, "B").Value = 0#
        wsR.Cells(rowIndexR, "C").Value = 0
        wsR.Cells(rowIndexR, "E").Value = "ZADANIE Z POPRZEDNIEJ DOBY"
        wsR.Cells(rowIndexR, "F").Value = "KONTYNUACJA"
        wsR.Cells(rowIndexR, "G").Value = "KONTYNUACJA"
        wsR.Cells(rowIndexR, "H").Value = "Czynności inne"
        wsR.Cells(rowIndexR, "J").Value = 0#
        wsR.Cells(rowIndexR, "K").Value = "S++"

        dtOrigNAR = 0#
        dtNewNAR = 0#
        vDateNAR = wsR.Cells(rowIndexR + 1, "A").Value
        vTimeNAR = wsR.Cells(rowIndexR + 1, "B").Value
        If IsNumeric(vDateNAR) Then
            dtOrigNAR = CDbl(vDateNAR)
        ElseIf IsDate(vDateNAR) Then
            dtOrigNAR = CDbl(CDate(vDateNAR))
        End If
        If IsNumeric(vTimeNAR) Then
            dtOrigNAR = dtOrigNAR + CDbl(vTimeNAR)
        ElseIf IsDate(vTimeNAR) Then
            dtOrigNAR = dtOrigNAR + CDbl(CDate(vTimeNAR))
        End If

        vDateNAR = wsR.Cells(rowIndexR, "A").Value
        If IsNumeric(vDateNAR) Then
            dtNewNAR = CDbl(vDateNAR)
        ElseIf IsDate(vDateNAR) Then
            dtNewNAR = CDbl(CDate(vDateNAR))
        End If
        wsR.Cells(rowIndexR, "I").Value = dtOrigNAR - dtNewNAR
    Next idxR

    Application.CutCopyMode = False

    '<<<<<<<<<<<<<<<<<<<<<<<<< Zakres sesji dobowej i zmianowej - KONIEC >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
